...
class CategoryServiceTest {
  ...
  class FindAllCategories {
    ...
    void sortAscending_DefaultAndNull() {
      ...
      List<Category> cats =
          new ArrayList<>(List.of(cat("Zebra"), cat("ąĄĄ"), cat("abc"), cat("Żaba"), cat("bcd")));
      ...
      for (List<Category> res : List.of(sorted1, sorted2, sorted3)) {
        assertEquals(
            expected,
            res.stream().map(Category::getName).toList(),
            "Should be sorted ascending (polish collation)");
      }
    }
    ...
    void sortsHandlesDiacriticsAndNulls() {
      ...
      List<String> expected =
          Arrays.asList("aBc", "Ąbc", "óbłęd", "Zebra", "żółw", null); // null ostatni
