...
class TaskServiceTest {
    ...
    void getAllTasks_DefaultsPaginationAndSort() {
        ...
        when(taskRepository.searchTasksByFilter(eq(userId), isNull(), isNull(), isNull(), isNull(), isNull(), any(PageRequest.class)))
                .thenReturn(expected);
        ...
        verify(taskRepository).searchTasksByFilter(eq(userId), isNull(), isNull(), isNull(), isNull(), isNull(), prCaptor.capture());
    ...
    void getAllTasks_DescendingSort() {
        ...
        ArgumentCaptor<PageRequest> prCaptor = ArgumentCaptor.forClass(PageRequest.class);
        verify(taskRepository).searchTasksByFilter(any(), any(), any(), any(), any(), any(), prCaptor.capture());
        PageRequest pageRequest = prCaptor.getValue();
        assertEquals(Sort.Direction.DESC, Objects.requireNonNull(pageRequest.getSort().getOrderFor("someField")).getDirection());
    }
    ...
    void getAllTasks_WithDueAfterAndDueBefore() {
        ...
        verify(taskRepository).searchTasksByFilter(
                eq(userId), isNull(), isNull(), isNull(),
                eq(LocalDate.of(2023, 1, 15).atStartOfDay()),
                eq(LocalDate.of(2023, 2, 5).atTime(LocalTime.MAX)),
                any(PageRequest.class));
    ...
    void getAllTasks_WithValidStatus() {
        ...
        when(userService.getCurrentUser()).thenReturn(mockUser);
        when(taskRepository.searchTasksByFilter(eq(userId), isNull(), eq(Status.TODO), isNull(), isNull(), isNull(), any(PageRequest.class)))
                .thenReturn(Page.empty());

        taskService.getAllTasks(filter);

        verify(taskRepository).searchTasksByFilter(eq(userId), isNull(), eq(Status.TODO), isNull(), isNull(), isNull(), any(PageRequest.class));
    }
    ...
    void getAllTasks_WithInvalidStatus() {
        ...
        verify(taskRepository).searchTasksByFilter(eq(userId), isNull(), isNull(), isNull(), isNull(), isNull(), any(PageRequest.class));
    }
    ...
    void getAllTasks_WithCategoryId() {
        ...
        when(userService.getCurrentUser()).thenReturn(mockUser);
        when(taskRepository.searchTasksByFilter(eq(userId), isNull(), isNull(), eq(categoryId), isNull(), isNull(), any(PageRequest.class)))
                .thenReturn(Page.empty());

        taskService.getAllTasks(filter);

        verify(taskRepository).searchTasksByFilter(eq(userId), isNull(), isNull(), eq(categoryId), isNull(), isNull(), any(PageRequest.class));
    }
    ...
    void searchTasksByTitle_ShouldReturnFilteredTasks() {
        ...
        when(taskRepository.searchTasksByFilter(
                userId, keyword, null, null, null, null, pageRequest))
                .thenReturn(mockPage);
    ...
    void keywordIsBlankOrNull_TreatedAsEmptyString() {
        ...
        filterNull.setPage(0); filterNull.setSize(1);
        when(userService.getCurrentUser()).thenReturn(mockUser);
        ...
        filterBlank.setPage(0); filterBlank.setSize(1);
        when(taskRepository.searchTasksByFilter(any(), eq(""), any(), any(), any(), any(), any()))
                .thenReturn(Page.empty());
        taskService.searchTasksByTitle(filterBlank);

        verify(taskRepository, times(2)).searchTasksByFilter(any(), eq(""), any(), any(), any(), any(), any());
    }
    ...
    void sortPropertyAndSortDirection_AllReferences() {
        ...
        filter.setDirection(null);
        filter.setTitle("X");
        filter.setPage(0); filter.setSize(1);
        when(userService.getCurrentUser()).thenReturn(mockUser);
        ...
        ArgumentCaptor<PageRequest> prCaptor = ArgumentCaptor.forClass(PageRequest.class);
        verify(taskRepository, atLeastOnce()).searchTasksByFilter(any(), any(), any(), any(), any(), any(), prCaptor.capture());
        boolean descCaptured = prCaptor.getAllValues().stream()
                .anyMatch(pr -> pr.getSort().stream().anyMatch(o -> o.getProperty().equals("deadline")
    ...
    void dueAfterDueBeforeAreProcessed() {
        ...
        verify(taskRepository, times(4)).searchTasksByFilter(
    ...
    void status_MappingAndCatch() {
        ...
        taskService.searchTasksByTitle(filter);
        verify(taskRepository).searchTasksByFilter(
                eq(userId), any(), eq(Status.TODO), any(), any(), any(), any()
        );
        ...
        taskService.searchTasksByTitle(filter);
        verify(taskRepository).searchTasksByFilter(
                eq(userId), any(), isNull(), any(), any(), any(), any()
        );
    ...
    void getUpcomingTasks_ShouldReturnPage() {
        ...
        when(userService.getCurrentUser()).thenReturn(mockUser);
        when(taskRepository.findByUserIdAndDueDateIsNotNullOrderByDueDateAsc(eq(userId), any())).thenReturn(page);
